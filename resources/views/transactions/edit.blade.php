                ...
                <form method="POST" action="{{ route('transactions.update', $transaction) }}">
                    ...
                    <div class="form-grid form-grid-2">
                        <div class="form-group">
                            ...
                            <select name="watch_id" id="watch_id" class="form-select" required>
                                @foreach($watches as $watch)
                                    <option value="{{ $watch->id }}" data-price="{{ $watch->asking_price }}" {{ old('watch_id',$transaction->watch_id)==$watch->id?'selected':'' }}>
                    ...
                    <div id="trade_in_fields" class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6 hidden">
                        <div class="form-group md:col-span-2">
                            ...
                            <select name="trade_in_watch_id" class="form-select">
                                ...
                                @foreach($watches as $watch)
                                    <option value="{{ $watch->id }}" {{ old('trade_in_watch_id', $transaction->trade_in_watch_id) == $watch->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="trade_in_appraisal_value" class="form-input" value="{{ old('trade_in_appraisal_value', $transaction->trade_in_appraisal_value) }}" step="0.01" min="0">
                        ...
                        <div class="form-group md:col-span-3">
                            ...
                            <select name="trade_in_cash_from" class="form-select">
                                ...
                                <option value="company" {{ old('trade_in_cash_from', $transaction->trade_in_cash_from) === 'company' ? 'selected' : '' }}>Company adds cash</option>
                                <option value="client" {{ old('trade_in_cash_from', $transaction->trade_in_cash_from) === 'client' ? 'selected' : '' }}>Client adds cash</option>
                            </select>
